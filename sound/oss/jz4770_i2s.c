...
void audio_close_endpoint(audio_pipe *endpoint, int mode)
{
	...
	if (mode == NOMAL_STOP) {
		...
		if( last_read_node )    //add by caizb@starheld.com
		{
			put_audio_freenode(endpoint->mem, last_read_node);
			last_read_node = NULL;
		}
...
void register_jz_codecs(void *func)
{
	...
	for (i = 0; i < NR_I2S; i++) {
		if (the_codecs[i].codecs_ioctrl == 0) {
			...
			the_codecs[i].use_mic_line_flag = USE_NONE;
...
static int jz_i2s_write_mixdev(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	case '3':
		...
		break;
	case '4':
		if (codec_ioctrl(codec, CODEC_SET_RECORD_DATA_WIDTH, 24) >= 0) {
...
//		printk("\nSNDCTL_DSP_SETFMT ... set to %d\n", val);
	...
	case SOUND_PCM_READ_BITS:
		if (mode & CODEC_RMODE) {
			if(codec->record_format & (AFMT_S8 | AFMT_U8)){
					format = 8;
			}
			else if(codec->record_format & (AFMT_S16_LE | AFMT_S16_BE)){
					format = 16;
			}
			else if(codec->record_format & (AFMT_S24_LE  | AFMT_S24_BE )){
					format = 24;
			}else{
					format = 24;
			}
			rc = put_user(format , (int *)arg);
...
//	__cpm_start_aic1();
	...
	codec_ioctrl(the_i2s_controller->i2s_codec, CODEC_FIRST_OUTPUT, 0);
	{
		//int i;
		//for (i = 0; i < 2; i ++)
		{
	...
	msleep(1);
	__i2s_disable_replay();
	jz_codec_close(the_i2s_controller->i2s_codec, CODEC_WMODE);
