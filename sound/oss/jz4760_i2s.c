...
#define JZCODEC_RW_BUFFER_TOTAL		4  //4
...
#define   DEBUG_WRMODE  0
#define   DEBUG_RECORD  0

#if  (DEBUG_WRMODE && DEBUG_RECORD)
#error
#endif
...
*/
	...
	pbuff = (audio_node *)((unsigned int)mem + sizeof(audio_head));
	for (i = 0; i < count; i++) {
		pbuff->pBuf	= (unsigned int)mem + headlistsize + pagesize * i;
...
static irqreturn_t jz_i2s_dma_irq (int irq, void *dev_id)
{
	...
	*/
	if (err == 0) {
		...
		endpoint->handle(endpoint);//handle_out_endpoint_work
	} else {
		DPRINT_IRQ("!!!! ??? unknown !!!\n");
	}
...
//	printk("%%%% format = %d\n", format);
	...
	if (endpoint == &out_endpoint) {
		__i2s_set_transmit_trigger((I2S_FIFO_DEPTH - endpoint->dma.onetrans_bit / sound_data_width) / 2 - 1);
	}
...
int audio_resizemem_endpoint(audio_pipe *endpoint, unsigned int pagesize, unsigned int count)
{
	...
	{
		printk("audio: Same pagesize && count !\n");
		return 1;
	}
...
static int jz_audio_open(struct inode *inode, struct file *file)
{
	...
	if ((file->f_mode & FMODE_READ) && (controller->pin_endpoint)) {
		printk("\nmedive Audio read device is busy!\n");
		return -EBUSY;
	}
	if ((file->f_mode & FMODE_WRITE) && (controller->pout_endpoint)) {
		printk("\nmedive Audio write device is busy!\n");
		return -EBUSY;
	}
	...
	{
#if DEBUG_RECORD
		printk("\n DEBUG:----open /data/record_test.pcm-%s\tline:%d\n",__func__,__LINE__);
		f_test = filp_open("/usr/local/record_test.pcm", O_RDWR | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR);
		f_test_offset = 0;
#endif
...
//medive add
void jz4760b_i2s_set_external_codec(void)
{
...
void jz4760b_i2s_set_internal_codec(void)
{
...
EXPORT_SYMBOL(jz4760b_i2s_set_external_codec);
EXPORT_SYMBOL(jz4760b_i2s_set_internal_codec);
...
static ssize_t jz_audio_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	while (count >= pout_endpoint->fragsize) {
		bat_cnt = endpoint_put_userdata(pout_endpoint,&(buffer[usecount]),pout_endpoint->fragsize);
...
#else
	//dma is align 32,so count must align 32
	//1.waite available node
    //2.copy date success or fail
    //3.if date have write,start dma to send date;no data,direct return
    int diff;
	while (count > usecount) { //dma is 32 width
		diff = count - usecount;
		if(diff < pout_endpoint->fragsize)
			bat_cnt = endpoint_put_userdata(pout_endpoint,&(buffer[usecount]),diff);
		else
			bat_cnt = endpoint_put_userdata(pout_endpoint,&(buffer[usecount]),pout_endpoint->fragsize);
		...
		// Error occured.
			...
			// Has not prepared any data, return error.
			else {
				DPRINT("bat_cnt < 0, usecount == 0\n");
				return bat_cnt;
			}
...
//		interruptible_sleep_on(&endpoint->q_full);
	...
	if (node && (node_buff_cnt = node->end - node->start))
	{
		...
		if (count >= (size_t)node_buff_cnt)
		{
			...
			vfs_write(f_test, (void *)(node->pBuf + node->start),node_buff_cnt, &f_test_offset);
		...
		else
		{
			...
			vfs_write(f_test, (void *)(node->pBuf + node->start),count, &f_test_offset);
