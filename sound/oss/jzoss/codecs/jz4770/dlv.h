...
//widget_ctrl
#define CODEC_MODE_WC_MIC1       (0x1U << 0)
#define CODEC_MODE_WC_MIC2       (0x1U << 1)
#define CODEC_MODE_WC_LINE_IN    (0x1U << 2)
#define CODEC_MODE_WC_HP         (0x1U << 3)
#define CODEC_MODE_WC_LINE_OUT   (0x1U << 4)
#define CODEC_MODE_WC_JD         (0x1U << 5)
...
#define CODEC_MODE_WC_LINE_IN_V    (0x1U << 20)
#define CODEC_MODE_WC_HP_V         (0x1U << 21)
#define CODEC_MODE_WC_LINE_OUT_V   (0x1U << 22)
#define CODEC_MODE_WC_JD_V         (0x1U << 23)

//collision detection
#define CODEC_MODE_CD_GEN(a,b) ((CODEC_MODE_ ## a) << (CODEC_MODE_CD_ ## b))
#define CODEC_MODE_CD_GET_VALUE(a,b,c) (((a) >> (CODEC_MODE_CD_ ## b)) & (CODEC_MODE_ ## c))
#define CODEC_MODE_CD_ADC_SEL    0
#define CODEC_MODE_CD_HP_SEL     2
#define CODEC_MODE_CD_LO_SEL     4
#define CODEC_MODE_CD_MIC_STE    6

#define CODEC_MODE_0    (0x0U)
#define CODEC_MODE_1    (0x1U)
#define CODEC_MODE_2    (0x2U)
#define CODEC_MODE_3    (0x3U)

struct dlv_codec_mode{
    unsigned int id;
    //widget ctrl
    unsigned int widget_ctrl;
    //collision detection
    unsigned int cd_mask;
    //cd_value must be equal to spec.It will be used when setting route .
...
#define __dlv_get_sr_jack()  ((dlv_read_reg(DLC_STATUS) & DLC_SR_JACK_PRESENT) ? 0 : 1)
...
#define __dlv_lo_set_route(md)	dlv_update_reg(DLC_CR_LO, md, DLC_LO_SEL_MASK)
...
#define __dlv_hp_set_route(md) dlv_update_reg(DLC_CR_HP, md, DLC_HP_SEL_MASK)
...
#define __dlv_set_dac_lrswap() \
    do{\
        dlv_write_reg_bit(DLC_CR_DAC, 1, DLC_CR_DAC_LRSWAP);    \
                                      \
    }while(0)
...
#define __dlv_adc_set_route(md)	dlv_update_reg(DLC_CR_ADC, md, DLC_CR_IN_SEL_MASK)
...
//FIXME: jack ??
#define ICR_COMMON_MASK	(ICR_ALL_MASK & (~DLC_IMR_SCLR) & (~DLC_IMR_SCMC2) & (~DLC_IMR_JACK))
