			 ...
			 int count, int *eof, void *data)
{
	...
	if (__gpio_get_pin(GPIO_USB_DETE)) {
		...
		__gpio_as_irq_fall_edge(GPIO_USB_DETE);
		...
		__gpio_as_irq_rise_edge(GPIO_USB_DETE);
#endif
                          ...
                          int count, int *eof, void *data)
{
        ...
        if (__gpio_get_pin(MSC2_HOTPLUG_PIN))
                len += sprintf (page+len, "REMOVE\n");
        else
...
#ifdef  ADD_IMEM2
#define IMEM2_MAX_ORDER 11              /* max 2^11 * 4096 = 8MB */
static unsigned int jz_imem2_base;      /* physical base address of ipu memory */
static unsigned int allocated_phys_addr2 = 0;
static struct imem_list *imem2_list_head = NULL; /* up sorted by phys_start */
#endif
...
static void imem_free(unsigned int phys_addr)
{
	...
	while (imem) {
        ...
        while (imem) {
                if (phys_addr == imem->phys_start) {
                        if (imem == imem2_list_head) {
                                imem2_list_head = imem->next;
                        }
                        else {
                                imemp->next = imem->next;
                        }
...
#ifdef ADD_IMEM2
static int imem2_alloc(unsigned int order)
{
	int alloc_ok = 0;
	unsigned int start, end;
	unsigned int size = (1 << order) * PAGE_SIZE;
	...
	start = jz_imem2_base;
	end = start + (1 << IMEM2_MAX_ORDER) * PAGE_SIZE;
...
static int imem2_read_proc(char *page, char **start, off_t off,
			  int count, int *eof, void *data)
{
	int len = 0;
	unsigned int start_addr, end_addr, max_order, max_size;
	...
	start_addr = jz_imem2_base;
	end_addr = start_addr + (1 << IMEM2_MAX_ORDER) * PAGE_SIZE;
	...
	*tmp++ = allocated_phys_addr2;	/* address allocated by 'echo n > /proc/jz/imem' */
	*tmp = max_order;		/* max order of current free buffers */
...
static int imem2_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	unsigned int val;
	...
	if (val == 0xff) {
		...
		imem_free_all();
		ipu_del_wired_entry();
	} else if ((val >= 0) && (val <= IMEM2_MAX_ORDER)) {
		/* allocate 2^val pages */
		imem2_alloc(val);
	} else {
		/* free buffer which phys_addr is val */
...
//#ifndef CONFIG_ANDROID_PMEM
 ...
 #ifdef ADD_IMEM2
        jz_imem2_base = (unsigned int)__get_free_pages(GFP_KERNEL, IMEM2_MAX_ORDER);
        if (jz_imem2_base) {
                /* imem (IPU memory management) */
                res = create_proc_entry("imem2", 0644, proc_jz_root);
                if (res) {
                        res->read_proc = imem2_read_proc;
                        res->write_proc = imem2_write_proc;
                        res->data = NULL;
                }

                /* Set page reserved */
                virt_addr = jz_imem2_base;
                for (i = 0; i < (1 << IMEM2_MAX_ORDER); i++) {
                        SetPageReserved(virt_to_page((void *)virt_addr));
                        virt_addr += PAGE_SIZE;
                }

                /* Convert to physical address */
                jz_imem2_base = virt_to_phys((void *)jz_imem2_base);

                printk("Total %dMB memory2 at 0x%x was reserved for IPU\n",
                       (unsigned int)((1 << IMEM2_MAX_ORDER) * PAGE_SIZE)/1000000, jz_imem2_base);
        }
        else
