...
//dubug code by ylyuan
void dump_buf(const u8 *buf, int len)
{
	int i;
	for (i = 0; i < len; i++) {
		printk(" 0x%02x", buf[i]);
...
static struct mtd_partition partition_info[] = {
	...
	{name:"ROOTFS",
	 ...
	 use_planes: 0},
	{name:"CONFIG_RW",
	 offset:92 * 0x100000LL,
	 size:10 * 0x100000LL,
	 use_planes: 0},
	{name:"CACHE",
	 offset:102 * 0x100000LL,
	 size:20 * 0x100000LL,
	 use_planes: 0},
};
...
static int partition_reserved_badblocks[] = {
	2,			/* reserved blocks of mtd0 */
	2,			/* reserved blocks of mtd1 */
	5,			/* reserved blocks of mtd2 */
	5,			/* reserved blocks of mtd3 */
	2,			/* reserved blocks of mtd4 */
	20,			/* reserved blocks of mtd5 */
	5,			/* reserved blocks of mtd6 */
	10,			/* reserved blocks of mtd7 */
};

#elif defined(CONFIG_JZ_NANDSZ_128M_RFS_86M)
static struct mtd_partition partition_info[] = {
	{name:"BOOT",
	 offset:0 * 0x100000LL,
	 size:3 * 0x100000LL,
...
#else // CONFIG_JZ_NANDSZ_256M_RFS_214M
#warning "++++++++++ CONFIG_JZ_NANDSZ_256M_RFS_214M +++++++++++"
static struct mtd_partition partition_info[] = {
	{name:"BOOT",
	 offset:0 * 0x100000LL,
	 size:3 * 0x100000LL,
...
#endif				/* CONFIG_JZ_NANDSZ_128M_RFS_76M */
...
//		__ecc_cnt_dec((eccsize + oob_per_eccsize) * 2 + PAR_SIZE);
		__ecc_cnt_dec((eccsize) * 2 + PAR_SIZE); //ylyuan for ubi
#if defined(CONFIG_MTD_NAND_DMA)
		__ecc_dma_enable();
#endif
	}
...
//		__ecc_cnt_enc((eccsize + oob_per_eccsize) * 2);
		__ecc_cnt_enc((eccsize) * 2); //ylyuan for ubi
#if defined(CONFIG_MTD_NAND_DMA)
		__ecc_dma_enable();
#endif
	}
...
//	nand_sw_bch_ops(mtd, chip->oob_poi, 1); //ylyuan for ubi
...
#define __nand_addr(n)		(REG8(addrport) = (n))
	...
	for (i = 0; i < eccsteps; i++) {
		...
		if (stat < 0)
		{
			...
			printk("==>%s: dump data, page=%d, eccsteps=%d\n", __func__, global_page, i);
			//dump_buf(databuf+eccsize*i, 512);
			printk("==>%s: dump oob, page=%d, eccsteps=%d\n", __func__, global_page, i);
			//dump_buf(oobbuf+ecc_pos+eccbytes*i, 13);
 ...
 *************************************/
	...
	desc = dma_desc_enc;
	for (i = 0; i < eccsteps; i++) {
//		next = CPHYSADDR((u32)dma_desc_enc1) + i * (sizeof(jz_bdma_desc_8word)); //ylyuan for ubi
//		next = CPHYSADDR((u32)dma_desc_enc) + (i + 1) * (sizeof(jz_bdma_desc_8word));
		next = CPHYSADDR((u32)desc) + sizeof(jz_bdma_desc_8word);
 ...
 *************************************/
	...
	*pval_bch_dcs = BDMAC_DCCSR_DES8 | BDMAC_DCCSR_EN;	// set value for writing ddr to enable channel 1
	...
	//	next = CPHYSADDR((u32)dma_desc_dec1 + i * (sizeof(jz_bdma_desc_8word)));
		next = CPHYSADDR((u32)dma_desc_dec2 + i * (sizeof(jz_bdma_desc_8word))); //ylyuan for ubi
