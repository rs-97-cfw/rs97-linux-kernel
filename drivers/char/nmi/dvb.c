...
static DVBPRIV dvbpriv;
static DVBPRIV *pdvb = &dvbpriv; 
...
static void dvb_run_tuner(void *pv)
{
...
static int dvb_get_sq(void)
{
	...
	if (core != 0xf6) {
		...
		if ((lock & 0x3) != 0x3) {
			...
			if (uber >= ((umax * 2)/10000)) {
				sq = 2;
			} else if ((uber < ((umax * 2)/10000)) && (uber >= ((umax * 19)/100000))) {
				sq = 3;
			} else if (uber < ((umax * 19)/100000)) {
				sq = 4; 
			}
		}
	}
...
#ifdef _HAVE_FLOAT_
	nmi_debug(_VERB_, "status: (%d), (%d), (%d), (%d), (%d), snr (%f), AGC (%d) RFAGC (%02x), RFAGC1(%02x), FOft (%f), RSSI(%d)\n",
		pSts->bAGCLock, pSts->bSyrLock, pSts->bChcTrkLock, pSts->bTpsLock, pSts->bFecLock,
		pSts->dSnr, agcGain/*pSts->dAgc*/, lock, pSts->uRFAGC, pSts->dFreqOffset, pSts->rssi);
#else
	nmi_debug(_VERB_, "status: (%d), (%d), (%d), (%d), (%d), snr (%f), AGC (%d) RFAGC (%02x), RFAGC1(%02x), FOft (%f), RSSI(%d)\n",
		pSts->bAGCLock, pSts->bSyrLock, pSts->bChcTrkLock, pSts->bTpsLock, pSts->bFecLock,
		pSts->Snr, agcGain, lock, pSts->uRFAGC, pSts->rssi);
...
static void dvb_run_demod(void *pv)
{
	DVBRUN *p = (DVBRUN *)pv;
	int lock;
...
static void dvb_t_filter_add_pid(uint32_t pid)
{
	int i, found = 0;
	uint32_t adr, val;
...
static void dvb_t_filter_remove_pid(uint32_t pid)
{
	int i, found = 0;
	uint32_t adr, val;
...
//	val32 = 0x03;  //add by xyma
	...
	if (plnk->mode == 1) {								/* dvb-t */
		if (!plnk->tsmode) {
			...
			val32 &= ~(0xfff << 14);
			val32 |= ((plnk->tsz) << 14);						/* dvbt_dma_size */
		} else {
			val32 &= ~(0x1 << 13);					
		}
	} else {														/* dvb-h */
...
//	uint32_t wrap;
	...
	if (((status >> 3) & 0x1) && ((enable >> 3) & 0x1)) {
		...
		do {
			...
			val32 = rReg32(0xf2bc);
			nmi_debug(_INFO_, "[DVB]: After DMA, status (%08x)\n", val32);
			...
			if ((val32 >> 4) & 0x1) {
				...
				**/
				wReg32(0xf2bc, val32);
				/**
					read the interrupt status register again to see if there are more data to move.
				**/
				val32 = rReg32(0xf2bc);		
				nmi_debug(_INFO_, "[DVB]: after DMA done, intr status (%08x)\n", val32);
				/**
					if no more data, we can exit. otherwise, we should continue move the data.
				**/
				if (!((val32 >> 3) & 0x1))	/* DMA ready is clear, we can exit since no more data */ 
					break;
			}
...
{
	uint32_t chipid = 0 , val32;
	/* get the chip id */
        printk(KERN_INFO "maxueyue Before chipid = %d\n", chipid); //Added by weiming for Debug
#if 0
	while(1) {
	chipid = rReg32(0xb468);
        printk(KERN_INFO "After chipid = %d\n", chipid); //Added by weiming for Debug
	chipid &= 0x00ffffff;
	mdelay(3000);
	}
#endif
...
static int dvb_chip_init(void)
{
	...
	/* check the chip id */
        printk(KERN_INFO "Before dvb_get_chipid chip id...(%08x)!!\n", chipid); //weiming
	chipid = dvb_get_chipid();
        printk(KERN_INFO "After dvb_get_chipid!!\n"); //weiming
	pdvb->chipid = chipid;
        printk(KERN_INFO "After dvb_get_chipid!!\n"); //Added by weiming for Debug
...
static NMIDTVVTBL *dvb_init(void)
{
	...
	if (dvb_chip_init() < 0)
		return NULL;
        printk(KERN_INFO "After dvb_chip_init!!\n");
	...
	pchp->dtv.nmi_dvb_get_sq = dvb_get_sq;
       ...
       pchp->dtv.nmi_rreg32 = rReg32;//added by Ryan(weiming)@CCI
