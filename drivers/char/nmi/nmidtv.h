...
********************************************/
typedef struct {
	void (*nmi_config_tuner)(void *);
	void (*nmi_config_demod)(void *);
	...
	int (*nmi_dvb_get_sq)(void);
       ...
       uint32_t (*nmi_rreg32)(uint32_t);//added by Ryan(weiming)@CCI
} NMIDTVVTBL;

typedef struct {
	void (*nmi_write_reg)(uint32_t, uint8_t *, uint32_t);
	void (*nmi_read_reg)(uint32_t, uint8_t *, uint32_t);
