...
InitCAT6611(void)
{
    ...
    /* Medive  add for  read  it6610 */

#if 0
    unsigned char read_value = HDMITX_ReadI2C_Byte(REG_INT_MASK3) ;
    printk("Medive printk: read value is 0x%x\n",read_value);
...
CheckHDMITX(BYTE *pHPD, BYTE *pHPDChange) 
{
    ...
    //printk("reg:%x,val=0x%x\n",REG_SYS_STATUS,sysstat);
...
//    BYTE uc ;
    ...
    ///Tranmin: set enhancement to +35%
    //HDMITX_OrReg_Byte(REG_AFE_RING,0x0c) ;
...
SetNCTS(ULONG PCLK, ULONG Fs)
{
  ...
  //  printk("SetNCTS(%ld,%ld): MCLK = %ld\n",PCLK,Fs,MCLK) ;
    ...
    //Tranmin: PCLK is 27Mhz, AUDIO SAMPLE FREQ IS 44.1Khz, then CTS should be 30000(0X7530)
   // n =  30000;
   // HDMITX_WriteI2C_Byte(REGPktAudCTS0,(BYTE)((n)&0xFF)) ;
   // HDMITX_WriteI2C_Byte(REGPktAudCTS1,(BYTE)((n>>8)&0xFF)) ;
   // HDMITX_WriteI2C_Byte(REGPktAudCTS2,(BYTE)((n>>16)&0xF)) ;
   ...
   // HDMITX_OrReg_Byte(REG_PKT_SINGLE_CTRL, 0x02) ; // D[1] = 1, user define CTS
