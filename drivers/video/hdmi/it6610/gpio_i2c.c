...
////#define SIO_C          (GPD31)
////#define SIO_D          (GPD30)
//#define SIO_C          (32*3+31)
//#define SIO_D          (32*3+30)
//#endif
//#define SIO_C          (32 * 4 + 31) /*PE31*/
//#define SIO_D          (32 * 4 + 30) /*PE30*/
...
//#define write_gpio_bit_set(x)       __gpio_as_output1(x)
...
SYS_STATUS HDMITX_WriteI2C_Byte(BYTE RegAddr, BYTE d)
{
	...
	if (flag == 0){
		printk("Medive printk: it6610 write i2c failed!\n");
	}
...
SYS_STATUS HDMITX_ReadI2C_ByteN(BYTE RegAddr, BYTE *pData, int N)
{
 ...
 if (flag == 0){
	 printk("Medive printk: it6610 read i2c failed!\n");
 }
