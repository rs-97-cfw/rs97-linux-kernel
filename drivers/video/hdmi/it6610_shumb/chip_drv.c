...
//static BYTE I2CADR = 0x98 ;
//static BYTE I2CDEV = 0 ;
...
static BOOL bAudioAdjustSampleClock = FALSE;  // add by shumb
...
void PowerDownHDMITX(int mode)
{
    BYTE uc ;
    switch(mode){
    case '1':
    	HDMITX_WriteI2C_Byte(0x04, 0x1D) ;
    	uc = HDMITX_ReadI2C_Byte(0x05) ;
    	uc |= 1 ;
    	HDMITX_WriteI2C_Byte(0x05, uc ) ;
    	HDMITX_WriteI2C_Byte(0x61, 0x30) ;
    	HDMITX_WriteI2C_Byte(0x62, 0x44) ; 
	break;
    case '2':
    	HDMITX_WriteI2C_Byte(0x05, 0x1 ) ;
    	HDMITX_WriteI2C_Byte(0x61, 0x20) ;
    	HDMITX_WriteI2C_Byte(0x62, 0x44) ; 
	break;
    case '3':
    	HDMITX_WriteI2C_Byte(0x04, 0x1D) ;
...
#ifdef SUPPORT_SYNCEMB //SUPPORT_EMBSYNC //ssg add
...
ProgramSyncEmbeddedVideoMode(BYTE VIC, BYTE bInputSignalType)
{
    ...
    printk("ProgramSyncEmbeddedVideoMode(%d , %d)\n",VIC, bInputSignalType) ;
    ...
    {
        BYTE *pTimingGen ;
        printk("Generate Sync and DE by Sync\n") ;
        ...
        case 22:
            ...
            break ;
        default:
            printk("Not a supported timing for VIC %d to gen sync/DE from sync.\n",VIC) ;
            return FALSE ;                    
        }
...
BOOL ProgramDEGenModeByID(MODE_ID id,BYTE bInputSignalType)
{
	...
	printk("\n----DeGen_Table index = %d-----\n",i);
        ...
        Switch_HDMITX_Bank(0) ;	//printk("\n===========\n");
        HDMITX_WriteI2C_Byte(0x90,DeGen_Table[i].Reg90) ; printk("Reg0x90=%X \n",DeGen_Table[i].Reg90);
        HDMITX_WriteI2C_Byte(0x92,DeGen_Table[i].Reg92) ; printk("Reg0x92=%X \n",DeGen_Table[i].Reg92);
        HDMITX_WriteI2C_Byte(0x93,DeGen_Table[i].Reg93) ; printk("Reg0x93=%X \n",DeGen_Table[i].Reg93);
        HDMITX_WriteI2C_Byte(0x94,DeGen_Table[i].Reg94) ; printk("Reg0x94=%X \n",DeGen_Table[i].Reg94);
        HDMITX_WriteI2C_Byte(0x9A,DeGen_Table[i].Reg9A) ; printk("Reg0x9A=%X \n",DeGen_Table[i].Reg9A);
        HDMITX_WriteI2C_Byte(0x9B,DeGen_Table[i].Reg9B) ; printk("Reg0x9B=%X \n",DeGen_Table[i].Reg9B);
        HDMITX_WriteI2C_Byte(0x9C,DeGen_Table[i].Reg9C) ; printk("Reg0x9C=%X \n",DeGen_Table[i].Reg9C);
        HDMITX_WriteI2C_Byte(0x9D,DeGen_Table[i].Reg9D) ; printk("Reg0x9D=%X \n",DeGen_Table[i].Reg9D);
        HDMITX_WriteI2C_Byte(0x9E,DeGen_Table[i].Reg9E) ; printk("Reg0x9E=%X \n",DeGen_Table[i].Reg9E);
        HDMITX_WriteI2C_Byte(0x9F,DeGen_Table[i].Reg9F) ; printk("Reg0x9F=%X \n",DeGen_Table[i].Reg9F);
        return TRUE ;
...
//     if(bHDMI)
//     {
//         printk("InitVideoMode(): SetHDMIMode\n") ;
//         HDMITX_WriteI2C_Byte(REG_HDMI_MODE,B_HDMI_MODE) ;
//     }
//     else
//     {
//         printk("InitVideoMode(): SetDVIMode\n") ;
//         HDMITX_WriteI2C_Byte(REG_HDMI_MODE,B_DVI_MODE) ;
//     }
    SetHDMIMode(bHDMI) ;
...
DisableAudioOutput()
{
	...
	bAudioAdjustSampleClock = FALSE; // add by shumb;
}
...
#endif
BOOL
CheckHDMITX(BYTE *pHPD, BYTE *pHPDChange, BYTE *pSysStat, BYTE *pAudio_chk) 
{
    ...
    if((sysstat & 0xF0)!= 0x70)
    	printk("%s---reg:%x,val=0x%x\n",__func__,REG_SYS_STATUS,sysstat);
    ...
    if(sysstat & B_INT_ACTIVE)
    {
	    ...
	    printk("INT_Handler: reg%02x = %02x\n",REG_INT_STAT1,intdata1) ;

	    if( intdata1 & B_INT_DDCFIFO_ERR )
	    {
		    printk("DDC FIFO Error.\n") ;
		    ClearDDCFIFO() ;
	    }
	    ...
	    {
		    printk("interrupt Authenticate Done.\n") ;                
	    ...
	    {
		    printk("interrupt Authenticate Fail.\n") ;       
		    AbortDDC();   // @emily add 
		    HDCP_ResumeAuthentication() ;
	    }
#endif // SUPPORT_HDCP
    ...
    SetAudioChannel() ;
	if(pAudio_chk){
		if(bAudioAdjustSampleClock == TRUE)
			*pAudio_chk = 1;
		else
			*pAudio_chk = 0;
	}
...
SetInputMode(BYTE InputColorMode, BYTE InputSignalType)
{
    ...
    printk("SetInputColorMode(%02X, %02X)\n",InputColorMode,InputSignalType) ;
    ...
    {
        ucData |= B_2X656CLK ; printk("CCIR656 mode\n") ;
    }

    if( InputSignalType & T_MODE_SYNCEMB )
    {
        ucData |= B_SYNCEMB ; printk("Sync Embedded mode\n") ;
    }

    if( InputSignalType & T_MODE_INDDR )
    {
        ucData |= B_INDDR ; printk("Input DDR mode\n") ;
    }
...
SetCSCScale(BYTE bInputMode,BYTE bOutputMode)
{
    ...
    case F_MODE_YUV444:
        printk("Input mode is YUV444 ") ;
        switch(bOutputMode&F_MODE_CLRMOD_MASK)
        {
        case F_MODE_YUV444:
            printk("Output mode is YUV444\n") ;
            csc = B_CSC_BYPASS ;
            break ;

        case F_MODE_YUV422:
            printk("Output mode is YUV422\n") ;
...
//    BYTE uc ;
    ...
    printk("reg[%02X] = %02X",REG_AFE_DRV_CTRL,HDMITX_ReadI2C_Byte(REG_AFE_DRV_CTRL)) ;
    printk("reg[%02X] = %02X ",REG_AFE_XP_CTRL1,HDMITX_ReadI2C_Byte(REG_AFE_XP_CTRL1)) ;
    printk("reg[%02X] = %02X ",REG_AFE_XP_CTRL2,HDMITX_ReadI2C_Byte(REG_AFE_XP_CTRL2)) ;
    printk("reg[%02X] = %02X ",REG_AFE_IP_CTRL,HDMITX_ReadI2C_Byte(REG_AFE_IP_CTRL)) ;
...
SetAudioFormat(BYTE NumChannel, BYTE AudioEnable, ULONG SampleFreq, BYTE AudSWL, BYTE AudioCatCode)
{
    ...
    printk("SetAudioFormat(%d channel,%02X,SampleFreq %ldHz,AudSWL %d,%02X)\n", NumChannel,  AudioEnable,  SampleFreq,  AudSWL,  AudioCatCode) ;
...
AdjustAudioSampleClock()
{
	...
	bAudioAdjustSampleClock = TRUE; // add by shumb
}
...
ReadEDID(BYTE *pData, BYTE bSegment, BYTE offset, SHORT Count)
{
    ...
    // printk("ReadEDID(%08lX, %d, %d, %d)\n",(ULONG)pData, bSegment, offset, Count) ;
    if( !pData )
    {
        printk("ReadEDID(): Invallid pData pointer %08lX\n",(ULONG)pData) ;
        return ER_FAIL ;
    }

    if(HDMITX_ReadI2C_Byte(REG_INT_STAT1) & B_INT_DDC_BUS_HANG)
    {
    	printk("Called AboutDDC()\n") ;
        AbortDDC() ;
    ...
    while( RemainedCount > 0 )
    {
        ...
        printk("ReadEDID(): ReqCount = %d, bCurrOffset = %d\n",ReqCount, bCurrOffset ) ;
        ...
        for(TimeOut = 250 ; TimeOut > 0 ; TimeOut -- )
        {
            ...
            {
                printk("ReadEDID(): DDC_STATUS = %02X, fail.\n",ucdata) ;
                return ER_FAIL ;
            }
        }
        ...
        if( TimeOut == 0 )
        {
            printk("ReadEDID(): DDC =%02x, TimeOut. \n",ucdata) ;
            return ER_FAIL ;
        }
...
SetAVIInfoFrame(AVI_InfoFrame *pAVIInfoFrame)
{
	...
	printk("SetAVIInfo(): ") ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB1)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB2)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB3)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB4)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB5)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB6)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB7)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB8)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB9)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB10)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB11)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB12)) ;
    //printk("%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB13)) ;
	printk("\n") ;
...
HDCP_GetBCaps(PBYTE pBCaps ,PUSHORT pBStatus)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_DDC_DONE)
        {
            //printk("HDCP_GetBCaps(): DDC Done.\n") ;
            break ;
        }

        if(ucdata & B_DDC_ERROR)
        {
            printk("HDCP_GetBCaps(): DDC fail by reg16=%02X.\n",ucdata) ;
            return ER_FAIL ;
        }
    }
...
HDCP_GetBKSV(BYTE *pBKSV)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_DDC_ERROR)
        {
            printk("HDCP_GetBCaps(): DDC No ack or arbilose,%x,maybe cable did not connected. Fail.\n",ucdata) ;
            return ER_FAIL ;
        }
    }
...
HDCP_Authenticate()
{
    ...
    for( TimeOut = 50 ; TimeOut > 0 ; TimeOut-- )
    {
    	...
    	if(bHDMIMode)
    	{
    		...
    		{
    			printk("Sink HDCP in DVI mode over HDMI,do not authenticate and encryption. BCaps = %x BStatus = %x\n",BCaps,BStatus) ;
    		}
    	}
    	else
    	{
    		...
    		{
    			printk("Sink HDCP in HDMI mode over DVI,do not authenticate and encryption. BCaps = %x BStatus = %x\n",BCaps,BStatus) ;
    		}
    	}
    	DelayMS(5) ;
    }
    ...
    if(TimeOut == 0 )
    {
    	if(bHDMIMode)
    	{
    		if((BStatus & B_CAP_HDMI_MODE)==0)
    		{
    			printk("Not a HDMI mode,do not authenticate and encryption. BCaps = %x BStatus = %x\n",BCaps,BStatus) ;
    			return ER_FAIL ;
    		}
    	}
    	else
    ...
    {
        printk("Down Stream Count %d is over maximum supported number 6,fail.\n",(BStatus & M_DOWNSTREAM_COUNT)) ;
        return ER_FAIL ;
    }
    */
    ...
    if(revoked)
    {
        printk("BKSV is revoked\n") ; return ER_FAIL ;
    }
    printk("BKSV %02X %02X %02X %02X %02X is NOT %srevoked\n",BKSV[0],BKSV[1],BKSV[2],BKSV[3],BKSV[4],revoked?"not ":"") ;
    ...
    if((BCaps & B_CAP_HDMI_REPEATER) == 0)
    {
        ...
        for(TimeOut = 250 ; TimeOut > 0 ; TimeOut --)
        {
            ...
            if(ucdata & B_INT_AUTH_FAIL)
            {
                ...
                */
                printk("HDCP_Authenticate(): Authenticate fail\n") ;
                bAuthenticated = FALSE ;
                return ER_FAIL ;
            }
        }
        ...
        if(TimeOut == 0)
        {
             printk("HDCP_Authenticate(): Time out. return fail\n") ;
             bAuthenticated = FALSE ;
             return ER_FAIL ;
        }
        return ER_SUCCESS ;
...
void SHATransform(ULONG * h)
{
	...
	for (t = 0; t < 20; t++) {
		...
		// printk("%08lX %08lX %08lX %08lX %08lX\n",h[0],h[1],h[2],h[3],h[4]) ;
...
void SHA_Simple(void *p,LONG len,BYTE *output)
{
    ...
    for( i = 0 ; i < len ; i++ )
    {
        ...
        c <<= (3-(i%4))*8 ;
        w[t] |= c ;
        // printk("pBuff[%d] = %02x, c = %08lX, w[%d] = %08lX\n",i,pBuff[i],c,t,w[t]) ;
    }
    t = i/4 ;
    if( i%4 == 0 )
    {
...
{
    printk("HDCP_CancelRepeaterAuthenticate") ;
    HDMITX_WriteI2C_Byte(REG_DDC_MASTER_CTRL,B_MASTERDDC|B_MASTERHOST) ;
    AbortDDC() ;
...
HDCP_GetKSVList(BYTE *pKSVList,BYTE cDownStream)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_DDC_DONE)
        {
            printk("HDCP_GetKSVList(): DDC Done.\n") ;
            break ;
        }

        if(ucdata & B_DDC_ERROR)
        {
            printk("HDCP_GetKSVList(): DDC Fail by REG_DDC_STATUS = %x.\n",ucdata) ;
            return ER_FAIL ;
        }
        DelayMS(5) ;
    }
...
HDCP_GetVr(BYTE *pVr)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_DDC_DONE)
        {
            printk("HDCP_GetVr(): DDC Done.\n") ;
            break ;
        }

        if(ucdata & B_DDC_ERROR)
        {
            printk("HDCP_GetVr(): DDC fail by REG_DDC_STATUS = %x.\n",ucdata) ;
            return ER_FAIL ;
        }
        DelayMS(5) ;
    }
    ...
    if(TimeOut == 0)
    {
        printk("HDCP_GetVr(): DDC fail by timeout. ucdata=0x%x\n",ucdata) ;
        return ER_FAIL ;
    }
...
HDCP_Authenticate_Repeater()
{
	...
	DelayMS(2);
    HDCP_Auth_Fire();
	sDelayMS(550); // emily add for test
    ...
    for(TimeOut = 250*6 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(uc & B_INT_AUTH_FAIL)
        {
            ...
            HDMITX_WriteI2C_Byte(REG_SYS_STATUS,0) ;
			*/
            printk("HDCP_Authenticate_Repeater(): B_INT_AUTH_FAIL.\n") ;
            goto HDCP_Repeater_Fail ;
        }
        // emily add for test
