...
//#define HDMITX_INPUT_SIGNAL_TYPE (T_MODE_DEGEN| T_MODE_SYNCGEN)    // for DE and sync regenerating by input sync.
...
HDMITX_SetOutput(void)
{
    ...
    #ifdef SUPPORT_DEGEN
    printk("HDMITX_SetOutput ProgramDEGenModeByID()\n");
...
{
    BYTE HPD, HPDChange, SysStat,audio_chk=0;
    int stat = -1,out_flag=-1;
	
    CheckHDMITX(&HPD,&HPDChange,&SysStat,&audio_chk) ;
//	printk("HPDChange=%d,HPD=%d,SysStat=0x%x\n",HPDChange,HPD,SysStat);
    if( HPDChange )
    {
        if( HPD )
        {
			...
			{
			#ifdef FORCE_TO_HDMI_OUTPUT
			  	printk("EDID error! Force to HDMI Output!!!\n");
				bHDMIMode = TRUE;
				bAudioEnable = TRUE ;
	...
	//if(HPD && ((SysStat & 0xF0) == 0x70))	
	if(HPD && ((SysStat & 0xE0) == 0x60))	
		stat = 0;
	else
		stat = -1;

//	printk("HDMITX_DevLoopProc:out_flag=%d ,audio_chk=%d, stat=%d\n",out_flag,audio_chk,stat);
...
HDMITX_ChangeDisplayOption(HDMI_Video_Type OutputVideoTiming, HDMI_OutputColorMode OutputColorMode)
{
   //HDMI_Video_Type  t=HDMI_480i60_16x9;
    ...
    case HDMI_640x480p60:
        ...
        VIC = 1 ;
        VideoPixelClock = 25200000 ; 
//	VideoPixelClock = 21000000 ;//  ssg add
...
//     AviInfo.info.ActiveFormatAspectRatio = 8 ; // same as picture aspect ratio
    AviInfo.pktbyte.AVI_DB[1] = 8 ; //bit3 ~1 //ssg add
...
ParseEDID(void)
{
  ...
  #if 1
    printk("=============== Sink edid block 0 ==============\n");
    printk("    CheckSum = 0x%02X\n", CheckSum);
    for(jj=0; jj<128; jj++)
    {
        printk(" %02X", EDID_Buf[jj]);
        if ((jj%16 == 3)||(jj%16==7)||(jj%16==11)) 
            printk(" ");
        if (jj%16 == 15) 
            printk("\n");
    }
...
void HDMITX_Standby(int mode)
{
	PowerDownHDMITX(mode);
}
void HDMITX_DisableOutput(void)
{
	DisableVideoOutput();
	DisableAudioOutput();
}
