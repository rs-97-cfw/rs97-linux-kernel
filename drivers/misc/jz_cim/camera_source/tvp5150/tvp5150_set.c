...
static struct tvp5150_ctrl tvp5150_qctrl[] = {
	{
		.id = TVP5150_CTRL_BRIGHTNESS,
		.minimum = 0,
		.maximum = 255,
	}, {
		.id = TVP5150_CTRL_CONTRAST,
		.minimum = 0,
		.maximum = 255,
	}, {
		.id = TVP5150_CTRL_SATURATION,
		.minimum = 0,
		.maximum = 255,
	}, {
		.id = TVP5150_CTRL_HUE,
...
static inline struct tvp5150 *to_tvp5150(struct i2c_client *c)
{
	return i2c_get_clientdata(c);
}
...
int tvp5150_read(struct i2c_client *c, unsigned char addr)
{
	...
	if (1 != (rc = i2c_master_send(c, buffer, 1)))
		dprintk("i2c i/o error: rc == %d (should be 1)\n", rc);
	...
	dprintk("tvp5150: read 0x%02x = 0x%02x\n", addr, buffer[0]);
				 ...
				 unsigned char value)
{
	...
	buffer[0] = addr;
	buffer[1] = value;
	dprintk("tvp5150: writing 0x%02x 0x%02x\n", buffer[0], buffer[1]);
...
{
	int opmode=0;
	struct tvp5150 *decoder = to_tvp5150(c);
	int input = 0;
	unsigned char val;
	...
	dprintk("Selecting video route: route input=%i, output=%i "
			   ...
			   const int fields)
{
	...
	if (std == V4L2_STD_ALL) {
		dprintk("VBI can't be configured without knowing number of lines\n");
		return 0;
...
static int tvp5150_set_std(struct i2c_client *c, v4l2_std_id std)
{
	...
	dprintk("Set video std register to %d.\n", fmt);
	tvp5150_write(c, TVP5150_VIDEO_STD, fmt);
	return 0;
}

static int tvp5150_s_std(struct i2c_client *c, v4l2_std_id std)
{
	struct tvp5150 *decoder = to_tvp5150(c);
...
int tvp5150_sensor_reset(struct i2c_client *c)
{
	...
	if (msb_rom == 4 && lsb_rom == 0) { /* Is TVP5150AM1 */
		dprintk("tvp%02x%02xam1 detected.\n", msb_id, lsb_id);

		/* ITU-T BT.656.4 timing */
		tvp5150_write(c, TVP5150_REV_SELECT, 0);
	} else {
		if (msb_rom == 3 || lsb_rom == 0x21) { /* Is TVP5150A */
			dprintk("tvp%02x%02xa detected.\n", msb_id, lsb_id);
		} else {
			dprintk("*** unknown tvp%02x%02x chip detected.\n",
				  msb_id, lsb_id);
			dprintk("*** Rom ver is %d.%d\n", msb_rom, lsb_rom);
		}
	}
...
static int tvp5150_s_ctrl(struct i2c_client *c, struct tvp5150_control *ctrl)
{
	u8 i, n;
...
static int tvp5150_s_fmt(struct i2c_client *c, struct v4l2_format *fmt)
{
	...
	if (svbi->service_set != 0) {
		for (i = 0; i <= 23; i++) {
			...
			svbi->service_lines[0][i] =
				tvp5150_set_vbi(c, vbi_ram_default,
...
{
	dprintk("tvp5150.c: removing tvp5150 adapter on address 0x%x\n",
		 c->addr << 1);
